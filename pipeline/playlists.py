...
def _spotify_error_detail(exc: spotipy.SpotifyException) -> str:
    ...
    if isinstance(exc.msg, dict):
        ...
        detail = str(exc.msg)
    if exc.reason:
        detail = f"{detail} {exc.reason}".strip() if detail else str(exc.reason)
    return detail.strip()
...
def _create_playlist_request(sp: spotipy.Spotify, user_id: str, name: str) -> dict:
...
def _create_playlist(
    ...
    try:
        pl = _create_playlist_request(sp, user_id, name)
        return pl["id"], name
        ...
        if _should_retry_with_ascii_name(exc, name, fallback_name):
            ...
            pl = _create_playlist_request(sp, user_id, fallback_name)
